#!/usr/bin/env python3
"""
Persona Council — API-based multi-persona review dispatcher via OpenRouter.

Dispatches reviews to 6 diverse models via OpenRouter API in bounded-parallel execution.
Same persona prompts as council_cli.py, with file contents injected for code-grounded reviews.

Usage:
  python3 council_api.py --plan path/to/plan.md --scenario prd --repo /path/to/project
  python3 council_api.py --plan path/to/plan.md --scenario dev-plan --repo /path/to/project
"""
...
MODELS = {
    "gpt-5.3-codex":    {"id": "openai/gpt-5.3-codex",          "in": 2.00, "out": 8.00},
    "deepseek-v3.2":    {"id": "deepseek/deepseek-v3.2",        "in": 0.30, "out": 0.80},
    "gemini-3.1-pro":   {"id": "google/gemini-3.1-pro-preview",  "in": 1.25, "out": 5.00},
    "qwen-3.5-27b":     {"id": "qwen/qwen3.5-27b",              "in": 0.20, "out": 0.60},
    "minimax-m2.5":     {"id": "minimax/minimax-m2.5",           "in": 0.30, "out": 1.20},
    "glm-5":            {"id": "z-ai/glm-5",                     "in": 0.50, "out": 2.00},
}

# --- Scenario configs (persona → model key) ---

SCENARIOS = {
    "prd": [
        {"persona": "requirements-archaeologist", "model": "gpt-5.3-codex",  "label": "Requirements Archaeologist"},
        {"persona": "spec-prosecutor",            "model": "deepseek-v3.2",  "label": "Spec Prosecutor"},
        {"persona": "testability-auditor",         "model": "gemini-3.1-pro", "label": "Testability Auditor"},
        {"persona": "edge-case-hunter",            "model": "qwen-3.5-27b",  "label": "Edge Case Hunter"},
        {"persona": "qa-test-case-reviewer",       "model": "minimax-m2.5",  "label": "QA Test Case Reviewer"},
        {"persona": "acceptance-criteria-engineer", "model": "glm-5",         "label": "Acceptance Criteria Engineer"},
    ],
    "dev-plan": [
        {"persona": "architecture-stress-tester",  "model": "gpt-5.3-codex",  "label": "Architecture Stress Tester"},
        {"persona": "deployment-risk-assessor",    "model": "deepseek-v3.2",  "label": "Deployment Risk Assessor"},
        {"persona": "observability-advocate",      "model": "gemini-3.1-pro", "label": "Observability Advocate"},
        {"persona": "coupling-detector",           "model": "qwen-3.5-27b",  "label": "Coupling Detector"},
        {"persona": "estimate-calibrator",         "model": "minimax-m2.5",   "label": "Estimate Calibrator"},
        {"persona": "test-qa-strategy-auditor",    "model": "glm-5",          "label": "Test & QA Strategy Auditor"},
    ],
}
...
def build_repo_context(repo_path: str | None, plan_text: str) -> str:
    ...
    Levels the playing field with CLI personas that can read files via filesystem.
    """
    if not repo_path:
        return ""
    ...
    # If fewer than 3 files found from plan, add key repo files
    ...
    if not found_files:
        return build_repo_brief(repo_path)
...
def build_repo_brief(repo_path: str | None) -> str:
    """Fallback: file list only (no contents)."""
    if not repo_path:
        return ""
...
async def call_openrouter(
    ...
    headers = {
        ...
        "X-Title": "Persona Council Review",
    }
    ...
    for attempt in range(TRANSPORT_RETRIES + 1):
        try:
            ...
            # Calculate cost from hardcoded pricing
...
async def run_council(
    ...
    # Pre-compute repo context (file contents, not just list)
    ...
    # Dispatch all concurrent (bounded by semaphore)
...
def write_output(
    ...
    lines = [
        f"# Persona Council Review (API): {scenario}",
        "",
        f"**Plan:** `{plan_path}`",
    ]
    if repo_path:
    ...
    output_path = OUTPUT_DIR / f"{scenario}-api-{ts_file}.md"
    output_path.write_text("\n".join(lines))

    synthesis_path = OUTPUT_DIR / f"{scenario}-api-{ts_file}-synthesis.md"
    synthesis_path.write_text(write_synthesis(results, plan_path, wall_time))

    metrics_path = OUTPUT_DIR / f"{scenario}-api-{ts_file}-metrics.json"
...
def write_synthesis(results: list[ReviewResult], plan_path: str, wall_time: float) -> str:
    """Condensed synthesis brief — same format as CLI version."""
...
def build_metrics(results: list[ReviewResult], wall_time: float) -> dict:
    """Structured metrics for comparison."""
    return {
...
def main():
    parser = argparse.ArgumentParser(
        description="Persona Council — API-based multi-persona review via OpenRouter"
    )
    ...
    print(f"Council (API): {args.scenario} | {len(SCENARIOS[args.scenario])} reviewers | "
    ...
    for r in SCENARIOS[args.scenario]:
        model_cfg = MODELS[r["model"]]
        print(f"  {r['label']:35s} → {r['model']} ({model_cfg['id']})")
    print()
